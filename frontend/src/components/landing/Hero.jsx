    ...
    <section id="home" className="pt-32 pb-20 px-6 min-h-screen flex items-center">
      <div className="max-w-7xl mx-auto w-full">
        <div className="text-center">
          ...
          <p className={`text-xl md:text-2xl mb-4 max-w-4xl mx-auto ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
            ...
            <button 
              ...
              className={`w-full sm:w-auto px-8 py-4 rounded-xl text-lg font-semibold transition ${isDark ? 'bg-gray-800 text-white hover:bg-gray-700 border border-gray-700' : 'bg-white text-gray-900 hover:bg-gray-50 border border-gray-300 shadow-md'}`}
            >
              🔍 View Dashboard
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto">
            <div className={`backdrop-blur-sm rounded-xl p-6 ${isDark ? 'bg-gray-800/50 border border-gray-700' : 'bg-white border border-gray-200 shadow-lg'}`}>
              <div className="text-4xl font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent mb-2">99%+</div>
              <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Detection Accuracy</div>
            </div>
            <div className={`backdrop-blur-sm rounded-xl p-6 ${isDark ? 'bg-gray-800/50 border border-gray-700' : 'bg-white border border-gray-200 shadow-lg'}`}>
            ...
            <div className={`backdrop-blur-sm rounded-xl p-6 ${isDark ? 'bg-gray-800/50 border border-gray-700' : 'bg-white border border-gray-200 shadow-lg'}`}>
              ...
              <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Fraud Detection</div>
            </div>
            <div className={`backdrop-blur-sm rounded-xl p-6 ${isDark ? 'bg-gray-800/50 border border-gray-700' : 'bg-white border border-gray-200 shadow-lg'}`}>
              <div className="text-4xl font-bold bg-gradient-to-r from-orange-400 to-red-400 bg-clip-text text-transparent mb-2">24/7</div>
              <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Monitoring</div>
            </div>
          </div>
        </div>
      </div>
