    ...
    <section id="dashboard-preview" className={`py-20 px-6 ${isDark ? '' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className={`text-4xl md:text-5xl font-bold mb-4 ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>Powerful Analytics Dashboard</h2>
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-center">
          ...
          <div className={`rounded-2xl p-8 shadow-2xl ${isDark ? 'bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-300' : 'bg-white border border-gray-200'}`}>
            <div className={`rounded-xl overflow-hidden ${isDark ? 'bg-gray-50 border border-gray-300' : 'bg-gray-50 border border-gray-200'}`}>
              {/* Mock Dashboard UI */}
              <div className={`p-4 flex items-center justify-between ${isDark ? 'bg-white border-b border-gray-300' : 'bg-gray-100 border-b border-gray-200'}`}>
                <div className="flex items-center space-x-2">
                  <div className="w-3 h-3 bg-red-500 rounded-full"></div>
                  <div className="w-3 h-3 bg-yellow-500 rounded-full"></div>
                  <div className="w-3 h-3 bg-green-500 rounded-full"></div>
                </div>
                <span className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>Dashboard</span>
              </div>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className={`rounded-lg p-3 ${isDark ? 'bg-blue-500/10 border border-blue-500/30' : 'bg-blue-50 border border-blue-200'}`}>
                    ...
                    <div className={`text-xl font-bold ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>1,234</div>
                  </div>
                  <div className={`rounded-lg p-3 ${isDark ? 'bg-red-500/10 border border-red-500/30' : 'bg-red-50 border border-red-200'}`}>
                    ...
                    <div className={`text-xl font-bold ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>23</div>
                  </div>
                  <div className={`rounded-lg p-3 ${isDark ? 'bg-green-500/10 border border-green-500/30' : 'bg-green-50 border border-green-200'}`}>
                    ...
                    <div className={`text-xl font-bold ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>1,211</div>
                  </div>
                </div>

                {/* Chart Placeholder */}
                <div className={`rounded-lg p-4 ${isDark ? 'bg-white border border-gray-300' : 'bg-white border border-gray-200'}`}>
                  <div className={`text-sm mb-3 ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>Fraud Detection Trends</div>
                ...
                {/* Recent Transactions */}
                <div className={`rounded-lg p-4 ${isDark ? 'bg-white border border-gray-300' : 'bg-white border border-gray-200'}`}>
                  <div className={`text-sm mb-3 ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>Recent Transactions</div>
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={i} className="flex items-center justify-between text-xs">
                        <span className={isDark ? 'text-gray-900' : 'text-gray-900'}>{tx.amount}</span>
                        <span className={`px-2 py-1 rounded ${tx.status === 'fraud' ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>
                          {tx.status}
                        </span>
                        <span className={isDark ? 'text-gray-500' : 'text-gray-600'}>{tx.prob}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-6">
            <h3 className={`text-3xl font-bold mb-6 ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>Dashboard Features</h3>
            ...
            {[
              ...
              <div key={index} className={`flex items-start space-x-4 rounded-xl p-4 transition ${isDark ? 'bg-gray-100 border border-gray-300 hover:border-blue-500/50' : 'bg-white border border-gray-200 hover:border-blue-400 shadow-sm'}`}>
                <div className="text-4xl">{feature.icon}</div>
                <div>
                  <h4 className={`font-semibold mb-1 ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>{feature.title}</h4>
                  <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>{feature.description}</p>
                </div>
              </div>
            ))}
