    ...
    <section id="how-it-works" className={`py-20 px-6 ${isDark ? '' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className={`text-4xl md:text-5xl font-bold mb-4 ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>How It Works</h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-8 relative z-10">
            {steps.map((step, index) => (
              <div key={index} className="relative">
                <div className={`backdrop-blur-sm border rounded-2xl p-6 transition-all duration-300 ${isDark ? 'bg-white/80 border-gray-300 hover:border-blue-500/50 hover:shadow-xl' : 'bg-white border-gray-200 hover:border-blue-400 shadow-md hover:shadow-xl'}`}>
                  {/* Step Number */}
                  ...
                  {/* Content */}
                  <h3 className={`text-xl font-semibold mb-3 text-center ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>{step.title}</h3>
                  <p className={`text-sm text-center leading-relaxed ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>{step.description}</p>
                </div>
        ...
        <div className={`mt-16 rounded-2xl p-8 border ${isDark ? 'bg-gradient-to-br from-blue-500/10 to-purple-500/10 border-blue-500/30' : 'bg-gradient-to-br from-blue-50 to-purple-50 border-blue-200'}`}>
          <h3 className={`text-2xl font-bold mb-6 text-center ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>Technical Architecture</h3>
          <div className="grid md:grid-cols-3 gap-6">
            <div className="text-center">
              <div className="text-4xl mb-3">🔬</div>
              <h4 className={`font-semibold mb-2 ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>Quantum Layer</h4>
              <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>PennyLane quantum circuits with 4 qubits for pattern recognition</p>
            </div>
            <div className="text-center">
              <div className="text-4xl mb-3">🤖</div>
              <h4 className={`font-semibold mb-2 ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>Classical Layer</h4>
              <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>XGBoost ensemble model for traditional feature analysis</p>
            </div>
            <div className="text-center">
              <div className="text-4xl mb-3">⚡</div>
              <h4 className={`font-semibold mb-2 ${isDark ? 'text-gray-900' : 'text-gray-900'}`}>Hybrid Fusion</h4>
              <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>Neural network combines quantum and classical outputs</p>
            </div>
          </div>
        </div>
      </div>
